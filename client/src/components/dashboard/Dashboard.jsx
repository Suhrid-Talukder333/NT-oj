...
const Dashboard = (props) => {
  ...
  useEffect(() => {
    ...
    const userId = props.match.params.id || parseJwt(accessToken).sub || null;

    axios
      .get(`${BACK_SERVER_URL}/api/user/userId/${userId}`, {
  ...
  useEffect(() => {
    if (user.stats) {
      ...
      Object.keys(user.stats.verdicts).forEach((keyName) => {
        ...
        _submissions.push([keyName, user.stats.verdicts[keyName]]);
      });
